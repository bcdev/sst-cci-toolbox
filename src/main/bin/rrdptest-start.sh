...
if [ -z $jobs ]; then
    line=`qsub -l h_rt=47:00:00,s_fsize=10G,sages_2ppn=1 -pe memory-2G 2 -j y -cwd -o $MMS_LOG/rrdptest-$year-$month.out -N in-$year$month $MMS_HOME/bin/rrdptest-run.sh $year $month`
    echo $line
