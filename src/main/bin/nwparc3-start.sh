...
if [ -z $jobs ]; then
    ...
    do
        ...
        do
            ...
            line=`qsub -l h_rt=24:00:00 -j y -cwd -o $MMS_LOG/nwparc3-$year-$month-$part-$sensor.out -N a3-$year$month$part-$sensor $MMS_HOME/bin/nwparc3-run.sh $year $month $part $sensor`
            echo $line
