...
if [ -z $jobs ]; then
    line=`qsub -l h_rt=47:00:00,s_fsize=10G -pe memory-2G 2 -j y -cwd -o $MMS_LOG/rrdpalgsel-$year-$month.out -N in-$year$month $MMS_HOME/bin/rrdpalgsel-run.sh $year $month`
    echo $line
