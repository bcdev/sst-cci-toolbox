...
class PMonitor:
    ...
    def __init__(self, inputs, request='', hosts=[('localhost',4)], types=[], weights=[], swd=None, cache=None, logdir='.', simulation=False, delay=None, fair=True, script=None):
        ...
        with self._mutex:
            ...
            self._pool = ThreadPool(concurrency)
    ...
    def execute(self, call, inputs, outputs, parameters=[], priority=1, collating=True, logprefix=None):
        ...
        with self._mutex:
            ...
            self._created += 1
            request = WorkRequest(None, [call, self._created, parameters, None, outputs, None, logprefix], priority=priority, requestID=self._created)
            for o in outputs:
                ...
                if collating:
                    ...
                    for i in range(len(input_paths)):
                        if i == 0:
                            ...
                            self._created += 1
                            request = WorkRequest(self._process_step,
                    ...
                    if task.callable == self._translate_step or task.callable == self._process_step:
                        ...
                        for i in range(len(input_paths)):
                            if i == 0:
                                ...
                                self._created += 1
                                task = WorkRequest(self._process_step, \
