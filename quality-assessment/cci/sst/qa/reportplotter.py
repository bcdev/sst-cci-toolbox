...
FIG_2_CHECKS = {
    ...
    # @todo 1 tb/tb continue with masks 2019-03-21
...
class ReportPlotter:
    ...
    def plot_figure_2(self):
        ...
        filepath = os.path.join(self.get_figure_dirpath(), filename)
        ReportPlotter.plot_report(report, FIG_2_CHECKS, check_labels, reference_counts, plot_title, plot_label, filepath,
                                  legend_on=True)
    ...
    def _extract_labels_plot_1(report):
        ...
        for key, values in FIG_1_CHECKS.items():
            for check in values:
                if check in report:
