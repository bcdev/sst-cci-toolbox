...
class SvrWorkflow:
    ...
    def run(self, hosts=list([('localhost', 120)]), calls=list(), log_dir='trace', simulation=False):
        """
        ...
        for sensor in self._get_sensors_by_period():
            ...
            if period is not None:
                ...
                while date < end_date:
                    ...
                    job = Job('svr-start' + '-' + year + '-' + month + '-' + day + '-' + sensor_name, 'svr-start.sh', [], ['/svr/' + sensor_name],
                              [year, month, day, sensor_name, self.usecase, self.version, self.archive_root, self.report_root])
                    monitor.execute(job)
                    date = _next_day(date)
    ...
    def _execute_report_accumulation(self, monitor):
        ...
        for sensor_name in self._get_sensor_names():
            ...
            job = Job('svr-accumulate-start' + sensor_name, 'svr-accumulate-start.sh', ['/svr/' + sensor_name], ['/sum/' + sensor_name], [sensor_name, report_dirpath, summary_report_pathname])
            monitor.execute(job)
    ...
    def _execute_report_plotting(self, monitor):
        ...
        for sensor_name in self._get_sensor_names():
            report_dirpath = SvrRunner.get_report_dirpath(self.report_root, self.version, self.usecase, sensor_name)
            summary_report_pathname = self.get_summary_report_pathname(report_dirpath, sensor_name)
            job = Job('svr-plot-start' + sensor_name, 'svr-plot-start.sh', ['/sum/' + sensor_name], ['/plt/' + sensor_name], [self.get_usecase(), sensor_name, summary_report_pathname, report_dirpath])
            monitor.execute(job)

    def get_summary_report_pathname(self, report_dirpath, sensor_name):
        summary_report_pathname = os.path.join(report_dirpath, self.get_usecase() + '-' + sensor_name + '-summary.json')
