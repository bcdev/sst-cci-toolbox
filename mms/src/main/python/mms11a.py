...
primary_sensors = [('avhrr.n10', '1986-11-17', '1991-09-16'),
...
secondary_sensors = [('avhrr.n10', '1986-11-17', '1991-09-16'),
                     ('avhrr.n11', '1988-11-08', '1994-12-31'),
                     ('avhrr.n12', '1991-09-16', '1998-12-14'),
                     ('avhrr.n14', '1995-01-01', '2002-10-07'),
                     ('avhrr.n15', '1998-10-26', '2010-12-31'),
...
for (sensor, sensorstart, sensorstop) in primary_sensors:
...
hosts = [('localhost', 48)]
types = [('ingestion-start.sh', 24),
         ('sampling-start.sh', 24),
         ('clearsky-start.sh', 24),
         ('mmd-start.sh', 24),
         ('coincidence-start.sh', 24),
         ('nwp-start.sh', 24),
         ('matchup-nwp-start.sh', 24),
         ('gbcs-start.sh', 24),
    ...
    for month in months:
        ...
        for sensor, sensorstart, sensorstop in primary_sensors:
            ...
            pm.execute('sampling-start.sh',
                       ...
                       parameters=[year, month, sensor, str(samples_per_month), str(skip), usecase])
            ...
            for sensor2 in secondary_sensors:
                pm.execute('mmd-start.sh',
                           ['/clr/' + sensor + '/' + year + '/' + month],
            ...
            for sensor2 in secondary_sensors:
                pm.execute('mmd-start.sh',
                           ...
                           parameters=[year, month, sensor, 'mmd10a', usecase])
