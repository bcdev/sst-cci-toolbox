...
echo "`date -u +%Y%m%d-%H%M%S` submitting tasks for arc3 $year/$month"
...
if [ -z $jobs ]; then
    ...
    do
        ...
        echo "`date -u +%Y%m%d-%H%M%S` submitting job arc3 $year/$month sensor $sensor"
        line=`bsub -P esacci_sst -oo $MMS_LOG/arc3-$year-$month-$sensor.out -eo $MMS_LOG/arc3-$year-$month-$sensor.err -J arc3-$year$month-$sensor $MMS_HOME/bin/arc3-run.sh $year $month $sensor`
        echo $line
