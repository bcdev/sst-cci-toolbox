...
submit_job() {
    jobname=$1
    command=$2
    bsubmit="bsub -R rusage[mem=16384] -q lotus -n 1 -W 12:00 -P esacci_sst -cwd ${MMS_INST} -oo ${MMS_LOG}/${jobname}.out -eo ${MMS_LOG}/${jobname}.err -J ${jobname} ${MMS_HOME}/bin/${command} ${@:3}"
