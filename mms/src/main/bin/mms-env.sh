...
submit_job() {
    jobname=$1
    command=$2
    bsubmit="bsub -R rusage[mem=8192] -q lotus -m lotus241 -n 1 -W 6:00 -P esacci_sst -cwd ${MMS_INST} -oo ${MMS_LOG}/${jobname}.out -eo ${MMS_LOG}/${jobname}.err -J ${jobname} ${mms.home}/bin/${command} ${@:3}"
