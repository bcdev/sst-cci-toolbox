...
submit_job() {
    ...
    jobname=$3
    command=$4
    bsubmit="bsub -R rusage[mem=${mem}] -m lotus241 -n 1 -W ${hrs} -P esacci_sst -cwd ${MMS_INST} -oo ${MMS_LOG}/${jobname}.out -eo ${MMS_LOG}/${jobname}.err -J ${jobname} ${mms_home}/bin/${command} ${@:5}"
