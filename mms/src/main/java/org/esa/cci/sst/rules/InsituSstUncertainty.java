        ...
        try {
            if (observationReader != null) {
                final ExtractDefinition extractDefinition = new ExtractDefinitionBuilder()
                        ...
                        .build();
                return observationReader.read("insitu.sst_uncertainty", extractDefinition);
            } else {
                throw new RuleException("Unable to read in-situ sst uncertainty");
            }
        } catch (IOException e) {
            throw new RuleException("Unable to read in-situ sst uncertainty", e);
        }
    }
